...
bool QuickEspNow::begin (uint8_t channel, uint32_t wifi_interface) {
    ...
    DEBUG_INFO (TAG, ARDUHAL_LOG_COLOR (ARDUHAL_LOG_COLOR_RED) "Starting ESP-NOW in in channel %u interface %s", channel, wifi_if == WIFI_IF_STA ? "STA" : "AP");
...
bool QuickEspNow::setChannel (uint8_t channel, wifi_second_chan_t ch2) {
    ...
    if ((err_ok = esp_wifi_set_channel (channel, ch2))) { // This is needed even in STA mode. If not done and using IDF > 4.0, the ESP-NOW will not work.
        DEBUG_DBG (TAG, "Error setting wifi channel: %d - %s", err_ok, esp_err_to_name (err_ok));
        return false;
    }
...
int32_t QuickEspNow::send (const uint8_t* dstAddress, const uint8_t* payload, size_t payload_len) {
    ...
    if (uxQueueMessagesWaiting (tx_queue) >= ESPNOW_QUEUE_SIZE) {
...
#endif // MEAS_TPUT
        DEBUG_DBG (TAG, "--------- %d Comms messages queued. Len: %d", uxQueueMessagesWaiting (tx_queue), payload_len);
...
int32_t QuickEspNow::sendEspNowMessage (comms_tx_queue_item_t* message) {
    ...
    DEBUG_DBG (TAG, "Peer added " MACSTR, MAC2STR (message->dstAddress));
    readyToSend = false;
    DEBUG_VERBOSE (TAG, "-------------- Ready to send: false");

    error = esp_now_send (message->dstAddress, message->payload, message->payload_len);
    DEBUG_DBG (TAG, "esp now send result = %s", esp_err_to_name (error));
    if (error != ESP_OK) {
        DEBUG_WARN (TAG, "Error sending message: %s", esp_err_to_name (error));
    }
    // if (error == ESP_OK) {
    //     txDataSent += message->payload_len;
    // }
...
void QuickEspNow::espnowTxHandle () {
    ...
    //DEBUG_WARN ("Process queue: Elements: %d", tx_queue.size ());
        ...
        while (xQueueReceive (tx_queue, &message, pdMS_TO_TICKS (1000))) {
            DEBUG_DBG (TAG, "Comms message got from queue. %d left", uxQueueMessagesWaiting (tx_queue));
            while (!readyToSend) {
                delay (1);
            }
        ...
        //message.payload_len = 0;
            DEBUG_DBG (TAG, "Comms message pop. Queue size %d", uxQueueMessagesWaiting (tx_queue));
        }
...
bool QuickEspNow::addPeer (const uint8_t* peer_addr) {
    ...
    if (!error) {
        DEBUG_DBG (TAG, "Peer added");
        peer_list.add_peer (peer_addr);
    } else {
        DEBUG_ERROR (TAG, "Error adding peer: %s", esp_err_to_name (error));
        return false;
    }
    DEBUG_DBG (TAG, "Peer " MACSTR " added on channel %u. Result 0x%X %s", MAC2STR (peer_addr), ch, error, esp_err_to_name (error));
    return error == ESP_OK;
}
...
void QuickEspNow::initComms () {
    ...
    tx_queue = xQueueCreate (ESPNOW_QUEUE_SIZE, sizeof (comms_tx_queue_item_t));
    xTaskCreateUniversal (espnowTxTask_cb, "espnow_loop", 8 * 1024, NULL, 1, &espnowTxTask, CONFIG_ARDUINO_RUNNING_CORE);

    rx_queue = xQueueCreate (ESPNOW_QUEUE_SIZE, sizeof (comms_rx_queue_item_t));
...
void QuickEspNow::espnowRxHandle () {
    ...
    if (xQueueReceive (rx_queue, &rxMessage, portMAX_DELAY)) {
        DEBUG_DBG (TAG, "Comms message got from queue. %d left", uxQueueMessagesWaiting (rx_queue));
        ...
        DEBUG_DBG (TAG, "No message in queue");
    }
...
void QuickEspNow::tx_cb (uint8_t* mac_addr, uint8_t status) {
    ...
    DEBUG_DBG (TAG, "-------------- Ready to send: true. Status: %d", status);
